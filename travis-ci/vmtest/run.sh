...
newest_rootfs_version() {
	{
	for file in "${!URLS[@]}"; do
		if [[ $file =~ ^${LIBBPF_ARCH}/${PROJECT_NAME}-vmtest-rootfs-(.*)\.tar\.zst$ ]]; then
			echo "${BASH_REMATCH[1]}"
		fi
	done
	} | sort -rV | head -1
...
download_rootfs() {
	...
	local dir="$2"
	download "${LIBBPF_ARCH}/${PROJECT_NAME}-vmtest-rootfs-$rootfsversion.tar.zst" |
...
case "$LIBBPF_ARCH" in
s390x)
	qemu=$VMTEST_ROOT/qemu/install/bin/qemu-system-s390x
	[[ -e "$qemu" ]] || "$VMTEST_ROOT"/build_qemu.sh "$VMTEST_ROOT"/qemu
	console="ttyS1"
	kvm_flags=()
	;;
x86_64)
	...
	kvm_flags=(-cpu kvm64 -enable-kvm)
	;;
esac
"$qemu" -nodefaults -display none -serial mon:stdio \
	"${kvm_flags[@]}" -smp "$(nproc)" -m 2G \
